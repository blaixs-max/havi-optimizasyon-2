...
def _optimize_single_depot(primary_depot: dict, all_depots: list, customers: list, vehicles: list, fuel_price: float) -> dict:
    ...
    try:
        ...
        # TIME DIMENSION: Add time constraints (max 1200 minutes per vehicle)
        ...
        routing.AddDimension(
            ...
            1320,  # maximum 1320 minutes per vehicle (1200 + slack)
        ...
        # Set maximum time for each vehicle (1200 target + 120 slack = 1320 max)
        for vehicle_id in range(num_vehicles):
            end_index = routing.End(vehicle_id)
            time_dimension.CumulVar(end_index).SetMax(1320)  # 1320 dakika max (1200 + slack)
        
        print(f"[OR-Tools] Time dimension added with 1200-minute limit per vehicle")
        ...
        # Increase timeout for complex problems (180s for better feasibility)
        ...
        print(f"[OR-Tools] Solving with PARALLEL_CHEAPEST_INSERTION + GLS (180s limit)...")
