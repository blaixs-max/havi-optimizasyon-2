...
async function optimizeWithRailway(
  ...
  try {
    ...
    const formattedRoutes = railwayResult.routes.map((route: any, index: number) => {
      ...
      // Use database's gen_random_uuid() instead of custom format
      // Routes table uses UUID PRIMARY KEY, so we don't set ID here
      // Database will auto-generate it
      const routeId = undefined // Will be generated by database
